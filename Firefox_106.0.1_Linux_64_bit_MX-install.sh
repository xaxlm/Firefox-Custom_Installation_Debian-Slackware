...
cd /
# Se descarga Firefox
wget https://download-installer.cdn.mozilla.net/pub/firefox/releases/133.0/linux-x86_64/es-MX/firefox-133.0.tar.bz2
cd /usr/local/
...
sleep 2
mv /firefox-133.0.tar.bz2 /usr/local/$DIRINST
tar jvxf firefox-133.0.tar.bz2
#
#App a Menú
cd /usr/share/applications/
touch firefox.desktop
echo '[Desktop Entry]' > firefox.desktop
echo 'Version=133.0' >> firefox.desktop
...
echo 'Icon=/usr/local/'$DIRINST'/firefox/browser/chrome/icons/default/default64.png' >> firefox.desktop
...
echo -ne '\n'
rm -rf /usr/local/$DIRINST/firefox-133.0.tar.bz2
